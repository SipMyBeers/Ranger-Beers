...
function setToken(token) { localStorage.setItem('rb_token', token); }
function clearToken() { localStorage.removeItem('rb_token'); localStorage.removeItem('rb_user'); }

function getUser() {
  try { return JSON.parse(localStorage.getItem('rb_user')); } catch { return null; }
}
function setUser(user) { localStorage.setItem('rb_user', JSON.stringify(user)); }
